<template>
  ...
  <v-row style="width: 100%; margin-bottom: 51px;    height: 79vh;background-color: white !important" class="mt-0">
    <v-data-table :headers="headers" :items="thisPlanningBoxes"
      ...
      :items-per-page="thisPlanningBoxesFiltered.length" style="width: 90%; margin: 0 auto">
...
">
                  ...
                  <v-data-table :headers="driverSelectionHeaderWithShift" :items="driversInPlanningWithCheck"
                    ...
                    <template v-slot:item="{ item }">
                      <tr :class="[
                        ...
                        <td>
                          <span class="fullname" :class="item.checked ? 'checkedDriverText' : ''">{{ Math.trunc(item.workingHours)
                            }}</span>
                        </td>
                    ...
                    <v-col cols="12" :style="{
                      ...
                      <v-row class="custom-scrollbar equipmentEdit" style="
                        ...
                        " class="equipmentForAdd" @click="setEquipmentAdd(null)">
                              ...
                              <td v-for="shift__time in shift__times" v-key="shift__time"><span class="timeClick">{{ shift__time.split("-")[0] }}<br>{{
                                shift__time.split("-")[1]
                                  }}</span></td>
                            </tr>
                          </tbody>
        ...
        <template v-slot:default="{ isActive }">
          <v-card class="dialogHeader pa-2">
            ...
            <div style="padding: 0 5%;">
              <PlanninfAsFileForDownload :actualShift="actualShift" :selectedDate="selectedDate" :landscape="landscape" :whShowInDownload="whShowInDownload" :perOneDriver="perOneDriver"
                :selectedPlanning="selectedPlanning" :ShiftIndex="ShiftIndex" :thisPlanningBoxesChunked="thisPlanningBoxesChunked" :thisPlanningBoxes="thisPlanningBoxes"
                :headers="headers" :splited="splited" @splitClicked="splitClicked"   @setLandscape="setLandscape"    @setWhShowInDownload="setWhShowInDownload"    @setPerOneDriver="setPerOneDriver"/>
            </div>
            <v-card-actions class="mt-5"> </v-card-actions>
          </v-card>
        </template>
  ...
  <template>
    <div v-if="landscape">
      <vue3-html2pdf
      :show-layout="false" :enable-download="true" :preview-modal="true" :filename="'ee'"
        :pdf-quality="2" :manual-pagination="true" ref="html2Pdf" :pdf-page-width="1123" :pdf-page-height="400"  pdf-content-width="100%"   pdf-orientation="landscape" >
...
export default {
  ...
  methods: {
    ...
    fixItForSTSLogic1(equipmentInPlanning_now) {
      console.log("yarbi salam equipmentInPlanning_now  jdida:",JSON.parse(JSON.stringify(equipmentInPlanning_now)));
      console.log("yarbi salam this.getOutToBeInserted  jdida:",JSON.parse(JSON.stringify(this.getOutToBeInserted)));
        ...
        (e) => {
          return (
            this.equipmentsInPlanningWithCheck.filter((c) => {
              return c.matricule == e.matricule && c.profile_group_id==this.getSelected_profile_group?.id;
            }).length == 0
          );
        }
      );
